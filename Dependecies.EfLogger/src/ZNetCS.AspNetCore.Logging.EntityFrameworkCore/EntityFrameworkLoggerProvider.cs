    ...
    where TContext : DbContext
{
    ...
    ///     The options.
    /// </param>
    public EntityFrameworkLoggerProvider(IServiceProvider serviceProvider,
        IOptions<EntityFrameworkLoggerOptions> options) : base(serviceProvider, options)
    {
    }
}
...
public class
    EntityFrameworkLoggerProvider<TContext, TLog> : EntityFrameworkLoggerProvider<TContext, TLog,
    EntityFrameworkLogger<TContext, TLog>>
    where TLog : Log<int>
    where TContext : DbContext
{
    /// <summary>
    ...
    /// </param>
    public EntityFrameworkLoggerProvider(IServiceProvider serviceProvider,
        IOptions<EntityFrameworkLoggerOptions<TLog>> options) : base(serviceProvider, options)
    {
    }
}
...
[SuppressMessage("ReSharper", "MemberCanBeProtected.Global", Justification = "Public API")]
public class
    EntityFrameworkLoggerProvider<TContext, TLog, TLogger> : EntityFrameworkLoggerProvider<TContext, TLog, TLogger, int>
    ...
    where TKey : IEquatable<TKey>
{
        ...
        Func<int, int, string, string, TLog>? creator = null)
    {
        ...
        factory = ActivatorUtilities.CreateFactory(
            typeof(TLogger),
            [
                typeof(string), typeof(Func<string, LogLevel, bool>), typeof(Func<int, int, string, string, TLog>)
            ]);
    }
    ...
    /// </param>
    public EntityFrameworkLoggerProvider(IServiceProvider serviceProvider,
        IOptions<EntityFrameworkLoggerOptions<TLog>> options)
    {
    ...
    {
        ThrowIfDisposed();
        return (ILogger)factory(serviceProvider, [categoryName, filter, creator]);
    }
}